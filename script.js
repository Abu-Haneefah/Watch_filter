...
searchInput.addEventListener("keyup", (e)=>{
    ...
    if(valueSearch){
        displayProduct(data.filter((item) => item.name.toLowerCase().indexOf(valueSearch) !== -1))
    }else{
        displayProduct(data)
    }
...
categoriesContainer.addEventListener("click", (e) =>{
    ...
    selectedText === "All"? displayProduct(data) : displayProduct(data.filter((item)=>item.cat === selectedText));
})
setCategories();
